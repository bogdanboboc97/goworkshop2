...
func (store *GormDataStore) GetAuthorsByFirstName(name string) ([]model.Author, error) {
	var authors []model.Author

	err := store.DBInstance.Where(&model.Author{FirstName: name}).Find(&authors).Error

	return authors, err
}

func (store *GormDataStore) GetAuthorsByLastName(name string) ([]model.Author, error) {
	var authors []model.Author

	err := store.DBInstance.Where(&model.Author{LastName: name}).Find(&authors).Error

	return authors, err
}

func (store *GormDataStore) GetAuthorsByName(name string) ([]model.Author, error) {
	var authors []model.Author

	err := store.DBInstance.Where("FirstName LIKE \"%" + name + "%\" OR Author.LastName LIKE \"%" + name + "%\"").Find(&authors).Error
